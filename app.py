...
def save_inventory(inventory):
    print("DEBUG: Saving Inventory with", len(inventory), "items.")  # Debugging line
...
def delete_product(product_id):
    print(f"DEBUG: Delete request received for Product ID: {product_id}") 
    inventory = load_inventory()
    print(f"DEBUG: Current Inventory Before Deletion: {inventory}") 

    # Ensure `product_id` is treated as a string to avoid type mismatches
    new_inventory = [item for item in inventory if str(item["Product ID"]) != str(product_id)]
    ...
    print(f"DEBUG: Inventory After Deletion: {new_inventory}") 
